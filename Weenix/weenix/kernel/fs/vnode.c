...
find:
        list_iterate_begin(&vnode_inuse_list, vn, vnode_t, vn_link) {
                dbg_print("Iteration! Current vnode no.: %d\n", vn->vn_vno);
                if ((vn->vn_fs == fs) && (vn->vn_vno == vno)) {
                        /* found it... */
                    dbg_print("Hit!! vnode no.: %d\n", vn->vn_vno);
...
{
        /*NOT_YET_IMPLEMENTED("VFS: special_file_read DONE");*/
