...
void init_call_all()
{
        ...
        dbg(DBG_THR,"ENTERED INIT_CALL_ALL\n");

        list_init(&funcs);
        buf = (char *) &kernel_start_init;
        end = (char *) &kernel_end_init;

        dbg(DBG_THR,"ENTERING WHILE LOOP NOW\n");
        ...
        dbg(DBG_THR,"EXITING LIST 1\n");

        list_iterate_begin(&funcs, func, struct init_function, if_link) {
                if (!func->if_called) {
                        func->if_search = ++_init_search_count;
                         dbg(DBG_THR,"ITERATION");
                        _init_call(&funcs, func);
                }
        } list_iterate_end();

        dbg(DBG_THR,"EXITING LIST 2\n");
        ...
        dbg(DBG_THR,"EXITING INIT_CALL_ALL\n");
